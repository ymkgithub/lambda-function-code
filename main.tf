...
resource "null_resource" "delay_on_function_code_changes" {
  # Use count or for_each if needed for multiple lambda functions
  for_each = var.lambda_functions

  # Trigger the null_resource when the source code hash changes
  triggers = {
    function_code_hash = aws_lambda_function.my_lambda[each.key].source_code_hash
  }

  # Use local-exec provisioner to introduce a delay
  provisioner "local-exec" {
    command = "sleep 180"  # Adjust delay time as needed
  }
...
resource "aws_lambda_provisioned_concurrency_config" "function_concurrency" {
  ...
  depends_on = [
    null_resource.delay_on_function_code_changes
  ]
}
